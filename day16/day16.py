    ...
    with open(filename) as file:
        ...
        if partOne:
            ...
            for row in range(len(lines)):
                numEnergizedTiles = getNumberOfEnergizedTiles(lines, row, 0, Direction.RIGHT)
                maxEnergizedTiles = max(maxEnergizedTiles, numEnergizedTiles)
                numEnergizedTiles = getNumberOfEnergizedTiles(lines, row, len(lines[row]) - 1, Direction.LEFT)
                maxEnergizedTiles = max(maxEnergizedTiles, numEnergizedTiles)
                print(f"Completed iterating over row {row} of {len(lines)}")
            for col in range(len(lines[0])):
                numEnergizedTiles = getNumberOfEnergizedTiles(lines, 0, col, Direction.DOWN)
                maxEnergizedTiles = max(maxEnergizedTiles, numEnergizedTiles)
                numEnergizedTiles = getNumberOfEnergizedTiles(lines, len(lines) - 1, col, Direction.UP)
                maxEnergizedTiles = max(maxEnergizedTiles, numEnergizedTiles)
...
def getNumberOfEnergizedTiles(lines, startRow, startCol, startDir):
    ...
    while len(beams) > 0:
        ...
        if currentCoord in allVisitedCoords and beam.dir in allVisitedCoords[currentCoord]:
